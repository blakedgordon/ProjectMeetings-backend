defmodule ProjectMeetings.User do
  ...
  def changeset(%User{} = user, params) do
    user
    |> cast(params, [:u_id, :display_name, :email, :google_token, :firebase_token, :meetings, :invites])
    ...
    |> validate_firebase_token(:firebase_token)
  end
  ...
  @doc """
  Add a specified meeting for the user
  """
  def add_meeting!(u_id, email, meeting) do
  ...
  @doc """
  Add a specified invite for the user
  """
  def add_invite!(u_id, email, meeting) do
  ...
  defp validate_google_token(changeset, :google_token) do
    validate_change changeset, :google_token, fn _, token ->
      ...
      do []
         ...
         _response ->
           [{:google_token, "Unrecognizable Token"}]
      end
    end
  end
  ...
  defp validate_firebase_token(changeset, :firebase_token) do
    validate_change changeset, :firebase_token, fn _, token ->
      with {:ok, data} <- decode_firebase_jwt(token),
        true <- valid_data?(data, @firebase_aud, @firebase_iss)
      do []
      else
         _response -> [{:firebase_token, "Unrecognizable Token"}]
      end
    end
  end
